...
sub render_file {
    ...
    if ( my $filepath = $args{filepath} ) {
        unless ( -f $filepath && -r $filepath ) {
            $self->app->log->error("render_file: Cannot read file [$filepath]. error [$!]");
        ...
        $self->app->log->error('render_file: You must provide "data" or "filepath" option');
        return;
    }
...
sub data_get {
    ...
    $self->app->log->info("data_get: Try to get data $dataname");
...
sub data_put {
    ...
    $self->app->log->info("data_put: Start put data to dir $datadir");

#    return $self->render(json => { datalist => undef, success => false, reason => 'datadir not exist' }) unless -d $datadir;
#    return $self->render(json => { datalist => undef, success => false, reason => 'cannot write to datadir' }) unless -w $datadir;
    ...
    foreach my $upload (@{$uploads}) {
        ...
        $self->app->log->info("data_put: Try to upload dataset $filename with datasize $filesize");

        my $dataname = $filename =~ s/^[ \.]+/_/gr;

        my $df = df("$datadir", 1);
        return $self->render(json => { success => false }) if $df->{bfree}+1 < $filesize;

        my $outfile = "$datadir/$dataname";

        $upload->move_to($outfile);

        $self->app->log->info("data_put: Move dataset $filename to filename $outfile");
...
##            unlink $outfile;
##            $self->app->log->info("data_put: Remove datafile $outfile because partial upload");
##            next;
##        }
        $self->app->log->info("data_put: Done upload dataset $filename to file $outfile");
        push @filenames, { name => $dataname, size => $filesize, realsize => $realsize, realname => $outfile };
    }

    $self->render(json => { datalist => \@filenames });
...
$r->add_condition(
    auth => sub {
        ...
        $log->info("$remote_address auth: Try auth as user ". $a->username($authstr) || undef);
        $a->auth($authstr);
    }
);
